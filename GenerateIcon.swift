...
class AppIconHelper {
    ...
    // 直接保存到AppIcon.appiconset目录
    func saveIconToAppIconSet() {
        // 获取项目目录
        let projectURL = URL(fileURLWithPath: "/Users/guangguang/Desktop/imdemo/MIDemo")
        let appIconsetURL = projectURL.appendingPathComponent("MIDemo/Assets.xcassets/AppIcon.appiconset")
        ...
        let iconURL = appIconsetURL.appendingPathComponent("AppIcon.png")
        ...
        do {
            try pngData.write(to: iconURL)
            print("✅ 图标已生成并保存到: \(iconURL.path)")
            print("✅ 请在Xcode中清理项目(Product > Clean Build Folder)并重新构建")
        } catch {
            print("❌ 保存图标失败: \(error.localizedDescription)")
        }
    }
...
// 主程序
print("正在生成应用图标...")
AppIconHelper.shared.saveIconToAppIconSet() 